...
<div>
    <div class="modern-card fade-in-up">
        <div class="card-header-modern">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="page-title">SPP / SPM LS</h3>
                    <p class="page-subtitle mb-0">Kelola SPP / SPM belanja langsung (LS)</p>
                </div>
                <div>
                    <button class="btn btn-modern-add" wire:click="openForm" data-toggle="modal"
                        ...
                        @foreach ($belanja as $row)
                            <tr wire:key="{{ $row->id }}">
                                ...
                                <td>
                                    @if ($row->no_spm_sipd)
                                        <span class="code-badge"
                                            style="font-size: 11px; word-break: break-all; display: inline-block; max-width: 100%;">{{ $row->no_spm_sipd }}</span>
                                    @else
                                ...
                                <td class="text-left" style="overflow: hidden;">
                                    <span title="{{ $row->uraian }}"
                                        style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4;">
                                        {{ $row->uraian }}
                                    </span>
                                </td>
                                <td class="text-right" style="white-space: nowrap;">
                                    <span class="amount-badge">Rp
                                        {{ number_format($row->total_nilai, 0, ',', '.') }}</span>
                                </td>
                                <td class="text-right">
                                    @if ($row->pajakLs->sum('nominal') > 0)
                                        <span class="badge badge-warning text-white mb-2" style="font-size: 11px;">Rp
                                            {{ number_format($row->pajakLs->sum('nominal'), 0, ',', '.') }}</span>
                                        <br>
                                        <a href="{{ route('pajakls', ['belanjaLsId' => $row->id]) }}"
                                            class="btn btn-xs btn-outline-secondary" title="Edit Pajak"
                                            style="border-radius: 4px;">
                                            <i class="fas fa-pencil-alt"></i> Edit
                                        </a>
                                    @else
                                        <a href="{{ route('pajakls', ['belanjaLsId' => $row->id]) }}"
                                            class="btn btn-sm btn-outline-primary" title="Rincian Pajak"
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="form-group row mb-3">
                            <label for="no_bukti" class="col-md-3 form-label" style="min-width: 150px;">No Bukti
                                (SPP)</label>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-4">
                                <input type="text" class="form-control"
                                    value="/Diskominfo/{{ $tahunTransaksi ?? date('Y') }}" readonly
                                    style="background-color: #f4f6f9;">
                            </div>
                        </div>
                        ...
                        <!-- No SPM SIPD -->
                        <div class="form-group row mb-3">
                            <label for="no_spm_sipd" class="col-md-3 form-label" style="min-width: 150px;">No SPM
                                SIPD</label>
                            <div class="col-md-9">
                                <input wire:model="no_spm_sipd" type="text" class="form-control" id="no_spm_sipd"
                                    placeholder="nomor sesuai dengan nomor SPM SIPD">
                                @error('no_spm_sipd')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                                            ...
                                            <tbody>
                                                @forelse ($rkas as $index => $rkaItem)
                                                    <tr>
                                                        <td>
                                                            <small
                                                                class="text-muted d-block">{{ $rkaItem['kode_belanja'] }}</small>
                                                        ...
                                                        <td>
                                                            ...
                                                            @if ($rkaItem['nilai'] > $rkaItem['initial_sisa'])
                                                                <small class="text-danger d-block mt-1">
                                                                    Max:
                                                                    {{ number_format($rkaItem['initial_sisa'], 0, ',', '.') }}
                                                                </small>
                                                            @endif
                                                        </td>
                                                    </tr>
                                                @empty
                                                    <tr>
                                                        <td colspan="3" class="text-center">Data RKAS tidak
                                                            tersedia.</td>
                                                    </tr>
                                                @endforelse
                                            </tbody>
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-md-9">
                                <textarea wire:model="uraian" class="form-control" id="uraian" placeholder="Masukkan uraian transaksi..."
                                    rows="3"></textarea>
                                @error('uraian')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
    ...
    <!-- View Print Modal -->
    <div wire:ignore.self class="modal fade" id="viewBelanja" tabindex="-1" aria-labelledby="viewBelanja"
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"
                        wire:click="closeModalPdf">Tutup</button>
                </div>
            </div>
        </div>
    </div>
