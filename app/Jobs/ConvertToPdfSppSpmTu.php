...
class ConvertToPdfSppSpmTu implements ShouldQueue
{
    ...
    {
        $pathWORD = storage_path('app/public/reports/spp-spm-tu/' . $this->nama_file_docx);
        $client = new Client();
        $response = $client->request('POST', 'http://10.90.237.12:8080/api/v1/convert/file/pdf', [
            'multipart' => [['name' => 'fileInput', 'contents' => fopen($pathWORD, 'r')]]
        ]);
